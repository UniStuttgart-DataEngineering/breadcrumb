...
class DBLPScenario7(spark: SparkSession, testConfiguration: TestConfiguration) extends DBLPScenario (spark, testConfiguration) {
  override def getName: String = "D7"
...
//    val inproceedings_select = inproceedings_filter.select($"_key".alias("ikey"), $"title._VALUE".alias("title"))
    ...
    val inproceedings_proceedings = proceedings_select.join(proceedings, $"crf" === $"pkey")
//    val inproceedings_proceedings_select = inproceedings_proceedings.select($"title", $"series_title")
    inproceedings_proceedings
